    ...
    def get_rvt_file_version(rvt_file):
      if op.exists(rvt_file):
        if olefile.isOleFile(rvt_file):
          ...
          file_info = file_info_read.decode('utf_16_le', "ignore")
          searchgroup = re.findall(r"(\d{4}).+?Build", file_info)
          version = re.search(r"(\d{4}).+?Build", file_info)
          if version is not None:
            rvt_file_version = re.search(r"(\d{4}).+?Build", file_info).group(1)
            msgbox = 'Fichier Revit version {}'.format(rvt_file_version)
            print(file_info)
          else:
            file_info2 = file_info_read.decode('utf_16_be', "ignore")
            searchgroup = re.findall(r"Format.+?(\d{4})", file_info2)
            version2 = re.search(r"Format.+?(\d{4})", file_info2)
            if version2 is not None: 
              rvt_file_version = re.search(r"Format.+?(\d{4})", file_info2).group(1)
              msgbox = 'Fichier Revit version {}'.format(rvt_file_version)
              print(file_info2)
            else:  
